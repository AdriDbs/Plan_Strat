...
const INDICATEURS = ['ETP_Workforce_Interne_FP', 'ETP_Workforce_Interne_M', 'Cout_ETP', 'ETP_Workforce_Externe_FP', 'ETP_Workforce_Externe_M', 'ETP_Besoin_Ressources'];
...
export const SyntheseExpansion: React.FC = () => {
  ...
  ), [data, filterEntite, filterHub]);

  const pivotRows = useMemo(() => {
    const map = new Map<string, PivotRowData>();
    for (const r of filtered) {
      if (!INDICATEURS.includes(r.Type_indicateur)) continue;
      const k = `${r.Entité}|${r.Hub}|${r.Process}|${r.CDR}|${r.Type_indicateur}`;
      if (!map.has(k)) {
        map.set(k, { key: k, Entité: r.Entité, Hub: r.Hub, Process: r.Process, CDR: r.CDR, Nom_CDR: r.Nom_CDR ?? '', Type_indicateur: r.Type_indicateur, values: {} });
      }
  ...
  const handleExport = () => {
    const rows = pivotRows.map(r => {
      const row: Record<string, unknown> = { Entité: r.Entité, Hub: r.Hub, Process: r.Process, CDR: r.CDR, Nom_CDR: r.Nom_CDR, Indicateur: r.Type_indicateur };
      YEARS.forEach(y => { row[String(y)] = r.values[y] ?? ''; });
      return row;
    });
    const ws = XLSX.utils.json_to_sheet(rows);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'SyntheseExpansion');
    XLSX.writeFile(wb, 'synthese_expansion.xlsx');
  };
  ...
  return (
    <div>
      ...
      <div style={{ border: '1px solid #1e2d45', borderRadius: 8, overflow: 'auto' }}>
          ...
          <tbody>
            {pivotRows.map((row, i) => (
              <tr key={row.key} style={{ background: i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.01)' }}>
                ...
                <td style={{ padding: '6px 12px', borderBottom: '1px solid rgba(30,45,69,0.4)', color: '#94a3b8', whiteSpace: 'nowrap' }}>{row.Process}</td>
        ...
        {pivotRows.length === 0 && <div style={{ textAlign: 'center', padding: '40px', color: '#475569', fontSize: 13 }}>Aucune donnée</div>}
      </div>
    </div>
  );
};
